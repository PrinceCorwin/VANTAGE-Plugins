...
{
    // Aggregated data per CWP from the CONST report
    ...
    internal class ConstImporter
    {
        ...
        // TODO: Update these column headers once you have the CONST report
        // These must match the Excel header row exactly (case-insensitive, whitespace-normalized)
        private static readonly string[] RequiredHeaders = { "CWP", "Quantity", "Status", "Shipped QTY" };

        // TODO: Update these constants if CONST vendor uses different activity metadata
        private const string ROCStep = "4.SHP";
        // Uses "CONST" suffix to distinguish from PTP records in the database
        private const string DescriptionPrefix = "FABRICATION - 4.SHP CONST ";
        private const string DescriptionPattern = "FABRICATION - 4.SHP CONST %";
        ...
        public async Task RunAsync(string filePath)
        {
            ...
            _host.LogInfo($"Parsed {cwpDataList.Count} unique CWPs from CONST report", "ConstImporter.RunAsync");
            ...
            _host.LogInfo($"CONST import: {created} created, {updated} updated, {unchanged} unchanged, {deletedCwps.Count} all-deleted", "ConstImporter.RunAsync");
        ...
        // Parse the CONST Excel report and aggregate quantities per CWP
        // TODO: Update column name lookups once you have the actual report
        private List<CwpData>? ParseReport(string filePath)
        {
            try
            {
                ...
                // TODO: Update these column lookups to match the CONST report headers
                ...
                // TODO: Update this header name if the CONST report uses a different date column
        ...
        // Find existing activities matching the CONST description pattern
        private async Task<Dictionary<string, ExistingRecord>> FindExistingRecordsAsync()
        {
            ...
            await Task.Run(() =>
            {
                ...
                cmd.CommandText = $"SELECT UniqueID, Description, AssignedTo, Notes, Quantity, PercentEntry, ActStart, ActFin FROM Activities WHERE Description LIKE '{DescriptionPattern}'";
